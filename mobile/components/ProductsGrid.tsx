...
const ProductsGrid = ({products, isLoading, isError}:ProductsGridProps) => {
  ...
  const renderProduct = ({item:product}:{item:Product})=>(
    <TouchableOpacity className='bg-surface rounded-3xl overflow-hidden mb-3' style={{width:"48%"}}
    activeOpacity={0.8} 
    onPress={()=>router.push(`/product/${product._id}`)}>
      <View className='relative'>
        <Image 
          ...
          resizeMode='cover'
        />
        <TouchableOpacity className='absolute top-3 right-3 bg-black/30 backdrop-blur-xl p-2 rounded-full'
        activeOpacity={0.7} 
      ...
      <View className='p-3'>
        ...
        <View className='flex-row items-center mb-2'>
          <Ionicons
            name='star' size={12} color={"#ffc107"}
          />
        ...
        <View className='flex-row items-center justify-between'>
          <Text className='text-primary font-bold text-lg'>Rs. {product.price.toFixed(2)}</Text>
...
function NoProductsFound(){
  <View className="py-20 items-center justify-center">
    ...
    <Text className='text-text-secondary text-sm mt-2'>Try adjusting you filters</Text>
  </View>
}